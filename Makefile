...
out_dirs := $(sort $(dir $(lib_objects) $(bin_objects) $(tests_objects)))
...
clean:
	...
	-$(RM) $(lib_objects) $(bin_objects) $(tests_objects) \
	       $(lib_depends) $(bin_depends) $(tests_depends) \
	       $(lib_autocpp) $(lib_autohpp) \
	       $(lib) $(bin) $(out_dir)/tests/tests
